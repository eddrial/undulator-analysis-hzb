...
print_name = ['flia_' int2str(first_run) '_' int2str(last_run) '.pdf'];
print_named = ['flia_' int2str(first_run) '_' int2str(last_run) '.png'];
title_name =  ['RUN: ' int2str(first_run) ' - ' int2str(last_run)];

%define PLOT YRANGE in Tmm
ymin = -4.;
ymax =  4.;
ymin1 = -0.2;
ymax1 =  0.1;
...
%fdat = fopen('mult_byzi_matlab_NB.dat','w');
fdat = fopen('mult_byzi_matlab_ZBG_M.dat','w');
for i=1:NFILES;
