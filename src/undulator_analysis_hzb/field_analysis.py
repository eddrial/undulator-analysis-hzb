...
def process_granit_bank(analysis_folder, output_file, component, ident, state, step):
    ...
    return 'Granite Bank Measurement Processed'
    
    
def process_moved_wire(analysis_folder, output_file, component, ident, state, step):
    files = glob.glob('{}/*.LOG'.format(analysis_folder))
    #this is shady... fails after 9999 ER 22.4.24
    run_number = files[0][-8:-4]
    
    save_moved_wire(analysis_folder, output_file, component, ident, state, step, run_number)
...
def save_moved_wire(analysis_folder, output_file, component, ident, state, step, run_number):
    meas_number = step
    run_number = run_number
...
#create the campaign data file
    ...
    #create the measurement type to a granite messbank measurement
    mes.moved_wire_measurement.convert_to_moved_wire_measurement(b)
    ...
    #TODO
    #create the measurement system and add the moved wire
    #moved_wire = ms.Measurement_System("Moved Wire")
    ...
    b.add_state(state)
    b.add_step('Step_{}'.format(meas_number))
    #b.add_measurement_system(moved_wire)
    ...
    #TODO
    #a.add_measurement_system(moved_wire)
